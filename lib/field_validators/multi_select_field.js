        ...
        var errors = inp.map(function (item) {
            ...
            if (!Array.isArray(this.options) && this.options.utilityInterface) {
                try {
                    ...
                    throw new Error("MultiSeletField can't find utility (" + this.options.utilityInterface.name + ", " + this.options.name + ")")
                }
                // TODO: Handle if we get a Promise from getOptions!
                var options = util.getOptions(inp, options, context)
                if (!async && options.then) {
                    throw new Error('MultiSeletField got a promise when fetching options from util, but validation wasn\'t async!')
