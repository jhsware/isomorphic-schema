    ...
    validate: function (inp, options, context, async) {
        ...
        inp.forEach(function (item, i) {
            if (!async) {
                ...
                // TODO: Perform async stuff
                var promise = this.valueType.validateAsync(item, options, context);
                if (!(promise && promise.then)) {
                    
                } else {
                    validationPromises.push(promise.then(function (tmpError) {
                        tmpValidationErrors[i] = {id: i, error: tmpError};
                        return Promise.resolve(tmpValidationErrors);
                    }))
                }
            }
        }.bind(this));
        ...
        if (!async || validationPromises.length === 0) {
            ...
            return Promise.all(validationPromises)
                .then(function (results) {
                    var tmpValidationErrors = results[0]
                    if (tmpValidationErrors.length === 0) {
