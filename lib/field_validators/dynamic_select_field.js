...
var IDynamicSelectField = require('../interfaces').IDynamicSelectField;

var DynamicSelectField = createObjectPrototype({
    implements: [IDynamicSelectField],
    ...
    validate: function (inp, options, context, async) {
        ...
        try {
            ...
            throw new Error("DynamicSelectField can't find utility (" + this.options.utilityInterface.name + ", " + this.options.name + ")")
        }
    ...
    getOptionTitle: function (inp) {
        ...
        return util.getOptionTitle(inp)
    }
});
