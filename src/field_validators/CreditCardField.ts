...
export class CreditCardField<T = TCreditCardField> extends BaseField<T> implements TCreditCardField {
  ...
  async validate(inp, options = undefined, context = undefined): Promise<TFieldError | undefined> {
    ...
    try {
      const result = _validateCreditCard(inp, this.cardValidationOptions);

      if (!(result.luhn_valid && result.length_valid)) {
        return {
          type: 'type_error',
          i18nLabel: i18n('isomorphic-schema--credit_card_field_incorrect_formatting', 'The card number is incorrectly entered'),
      ...
      return {
        ...
        i18nLabel: i18n('isomorphic-schema--credit_card_field_not_supported', 'Entered card type is not supported'),
  ...
  getMetaData(inp) {
    return _validateCreditCard(inp, this.cardValidationOptions);
  }

  toFormattedString(inp) {
    if (inp) {
      const pretty = (inp, type) => {
        var formatter = _cardFormatters[type] || _cardFormatters['default']
        return formatter(inp)
      }

      try {
        var res = _validateCreditCard(inp, this.cardValidationOptions)
      } catch (e) {
        // 
      }

      return pretty(inp, 'default');
    }
